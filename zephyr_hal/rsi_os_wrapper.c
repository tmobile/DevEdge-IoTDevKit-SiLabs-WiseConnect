...
{
	if (mutex == NULL || *mutex < 1 || *mutex > 32 || !(mut_map & BIT((*mutex - 1)))){
...
{
	if (mutex == NULL || *mutex < 1 || *mutex > 32){
		return RSI_ERROR_INVALID_PARAM;
	}
...
{
	if (semaphore == NULL || *semaphore < 1 || *semaphore > 64){
		return RSI_ERROR_INVALID_PARAM;
	}
...
rsi_error_t rsi_semaphore_check_and_destroy(rsi_semaphore_handle_t *semaphore)
{
	...
	if (*semaphore < 1 || *semaphore > 64 || !(sem_map & BIT((*semaphore - 1)))) {
		return RSI_ERR_NONE;
	}
...
{
	if (semaphore == NULL || *semaphore < 1 || *semaphore > 32){
		return RSI_ERROR_INVALID_PARAM;
	}
